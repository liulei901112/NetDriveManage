...
namespace NetDriveManage.Util
{
    ...
    public class AppUtil
    {
        ...
        static AppUtil()
        {
            ...
            {
                // 加载区域配置
                LoadAllKeyValue(AppConst.NET_DRIVE_CONFIG_KEY);

                List<string> netDriveList = ReadSectionList(AppConst.NET_DRIVE_CONFIG_KEY);
                if (netDriveList != null)
                {
                    foreach (string netDriveKey in netDriveList)
                    {
                        LoadAllKeyValue(netDriveKey);
                    }
                }
            });
        ...
        private static void LoadAllKeyValue(string section)
        {
                ...
                foreach (string entry in items)
                {
                    ...
                    try
                    {
                        lock (_AppIniCache)
                        {
                            if (_AppIniCache.ContainsKey(section))
                            {
                                sectionDic = _AppIniCache[section];
                                // 设置 section 节点子项
                                if (sectionDic.ContainsKey(v[0]))
                                {
                                    sectionDic[v[0]] = v[1];
                    ...
                    {
                        log.Error("子节点数据处理失败：" + section + " => key=" + v[0] + ", val=" + v[1]);
                    }
                }
            ...
            {
                log.Error("LoadAllKeyValue => " + section + " Error：" + ex.Message, ex);
            }
        }
        #endregion
